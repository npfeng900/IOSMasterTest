...
class TinyPixView: UIView {
    ...
    private func calculateGridForSize(size: CGSize) {
        ...
        gridRect = CGRectMake((size.width-space)/2, (size.height-space)/2, space, space)
    }
    ...
    override func drawRect(rect: CGRect) {
        if document != nil
        {
            ...
            if CGSizeEqualToSize(size, lastSize) == false
            {
                lastSize = size
    ...
    private func drawBlockAt(row row: Int, column: Int) {
        //多出来的1代表线
        let startX = gridRect.origin.x + gap + (blockSize.width + gap)*(7-CGFloat(column)) + 1
        let startY = gridRect.origin.y + gap + (blockSize.height + gap)*CGFloat(row) + 1
        
        let blockFrame = CGRectMake(startX, startY, blockSize.width-2, blockSize.height-2)
        let color = document.getState(row: row, column: column) ? UIColor.blackColor() : UIColor.whiteColor()
    ...
    private func toggleSelectedBlock() {
        if selectedBlockIndex.row != -1 && selectedBlockIndex.column != -1
        {
            document.toggleStateAt(row: selectedBlockIndex.row, column: selectedBlockIndex.column)
            document.undoManager?.prepareWithInvocationTarget(document).toggleStateAt(row: selectedBlockIndex.row, column: selectedBlockIndex.column)
            setNeedsDisplay()
        }
    }
    ...
    override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if touched.row != selectedBlockIndex.row || touched.column != selectedBlockIndex.column
        {
